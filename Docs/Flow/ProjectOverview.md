...
Phase 4 status (2026-06-10): CMS module completion — ADMIN content-block editor.
  ...
  - CONTENT-BLOCK KEY REGISTRY (admin Add-website-text dropdown = KNOWN_BLOCK_KEYS in CmsDeliveryManager.tsx;
    ...
    Site-read content-block keys as of 2026-06-11:
      • contact.phone / contact.whatsapp / contact.email / contact.city  → Footer.tsx, Contact.tsx, MobileActionBar.tsx
      • home.hero.eyebrow / home.hero.title / home.hero.subtitle          → Home.tsx hero (NEW 2026-06-11, each getBlock(...)?.content||fallback; title keeps styled <br>/italic default when no block)
      • home.cta.title / home.cta.subtitle                                → Home.tsx closing CTA (NEW 2026-06-11)
    NOT content blocks (different subsystems — do NOT add to the block dropdown):
      • home.hero / home.coverage = SCREEN-IMAGE slot keys (Screen Images tab → <SnapshotImage slotKey=…>).
      • service-content/{serviceKey} (e.g. general-service) = separate public per-service API /api/cms/public/service-content/{key}, not a getBlock block.
    Admin dropdown shows friendly labels; custom keys validated /^[a-z0-9]+(?:[.-][a-z0-9]+)*$/; content required (create + save) so a published block can't render blank. Remaining hardcoded Home copy (trust-strip stats, services-section heading, coverage, how-it-works steps) and About/WhyCoolzo/Services pages are still NOT CMS-wired — candidates for future blocks.
