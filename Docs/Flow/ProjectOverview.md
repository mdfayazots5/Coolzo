...
CATALOG-NATIVE SERVICE IMAGE (Admin Service & Equipment Catalog) — [IMPLEMENTED 2026-06-10]:
  ...
  Notes on Drift (cross-source — RECONCILIATION CHOSEN; canonical = tblService):
    ...
    PHASE 4 DONE [2026-06-15] (per-service AI image PROMPT — additive, non-destructive; owner: Prompt
      Engineer): admins can store a tuned, reusable AI image-generation prompt per service so AI tools
      stop producing off-brand / mis-framed images, and so there is no future rework.
      DB: added tblService.ImageAIPrompt (NVARCHAR(1024)/VARCHAR(1024) NULL; NULL = UI shows a generated
        suggested prompt). EF: Service.ImageAIPrompt + ServiceConfiguration HasMaxLength(1024). Migrations:
        SqlServer Docs/Database/SQL/20260615_Add_Service_ImageAIPrompt.sql, Postgres
        Docs/Postgres/21_add_service_imageaiprompt.sql (idempotent). [APPLY to Supabase prod before use.]
      API: new write endpoint PUT /api/admin/services/{serviceId}/image-prompt (Authorize
        Policy=lookup.manage), ServiceCatalogAdminController.
        Feature: ServiceCatalogAdmin/Commands/SetServicePrompt/SetServicePromptFeature.cs (mirrors
          SetServiceImage — loads tracked entity, sets ImageAIPrompt + UpdatedBy/LastUpdated, audit-logs
          "SetServicePrompt", SaveChanges; 404 when missing; null/empty clears the override).
        Request DTO: SetServicePromptRequest { ImageAIPrompt (string?, null/empty = clear) }.
        Response DTO: ServiceLookupResponse — extended with ImageAIPrompt (8th positional field, default ""
          → additive & safe for mobile/web consumers). GetServicesQueryHandler + SetServiceImageFeature
          now populate it. GET /api/booking-lookups/services therefore returns imageAIPrompt.
      Admin UI: shared component Frontend/Admin/src/components/shared/ImagePromptStudio.tsx — a small
        "Prompt" button (Sparkles icon, gold dot when a custom prompt is saved) placed beside Upload/Replace;
        opens a self-contained modal (w-[min(92vw,640px)], robust fixed sizing per CLAUDE.md §5b) with an
        editable textarea, Copy, Reset-to-suggested, and Save (persists via setServicePrompt). Default
        suggestion generated by Frontend/Admin/src/lib/image-prompts.ts buildServiceImagePrompt (deterministic,
        on-brand 1280×720/16:9, navy/gold palette, design-system guardrails — no posed-uniformed-staff stock).
        ServiceImagesScreen.tsx: added the Prompt button per service + a usage summary header
        (N services · with image · still need one · custom prompts saved). Repository: service-catalog-repository.ts
        AdminServiceItem.imageAIPrompt + setServicePrompt(serviceId, prompt|null).
      CMS images (CmsDeliveryManager.tsx, tblScreenImageSlot): the existing SuggestedAIPrompt (creative seed,
        already DB-backed via PUT /api/cms/admin/image-slots/{id}) is now editable+savable via the same
        ImagePromptStudio (edits the SEED only — technical constraints stay auto-appended by buildImagePrompt,
        so no double boilerplate); the composed final prompt remains shown + Copy. Added an Images-tab summary
        (slots · uploaded · empty · with saved prompt). saveSlotPrompt → updateImageSlot.
      DB APPLIED: tblService.ImageAIPrompt applied + verified on Supabase prod 2026-06-15 (varchar(1024),
        nullable) — endpoint live-capable.
      PROMPT ENGINE UPGRADE [2026-06-15] (owner: Prompt Engineer as Senior Commercial Photographer):
        image-prompts.ts is now a Gemini-optimised engine. Stored value = a SHORT SUBJECT SEED only
        (business intent); composeServicePrompt / composeSlotPrompt layer in DSLR commercial-photography
        craft (full-frame, 35mm f/1.8, shallow DoF, natural light, premium grading, authentic candid
        people / real skin & hands), navy/gold brand accents, slot-exact px+aspect constraints, then append
        the GLOBAL_QUALITY positive standard and GLOBAL_NEGATIVE negative prompt verbatim. Output is a
        two-part "Positive prompt: … / Negative prompt: …" block. This guarantees the global standards are
        applied to EVERY image (incl. edited ones) and keeps seeds < 1024 (no storage change). ImagePromptStudio
        now edits the seed and shows a live read-only full-prompt preview for copy (same model for services
        + CMS). Constants GLOBAL_QUALITY / GLOBAL_NEGATIVE are the single source of truth.
      Validation status: Admin tsc --noEmit clean (2026-06-15); Backend build 0/0 (prior round).
        UI rendered-check of the Prompt modal still PENDING (UNVERIFIED per §5b — auth wall).
    PHASE 5 DONE [2026-06-15] (AI image PROMPT extended to the Service Catalog manager — categories +
      services; additive, full persistence; owner: Prompt Engineer): the per-service prompt added in
      Phase 4 lived only on the dedicated ServiceImagesScreen. The Service Catalog CRUD screen
      (ServiceCatalogManager.tsx) — which is where category images AND service images are actually
      uploaded inside Add/Edit modals — had no Prompt button. Closed that gap so every image uploader
      (CMS slots, Service Images, Catalog category, Catalog service) carries the same ImagePromptStudio.
      DB: added tblServiceCategory.ImageAIPrompt (NVARCHAR(1024)/VARCHAR(1024) NULL; mirrors
        tblService.ImageAIPrompt). EF: ServiceCategory.ImageAIPrompt + ServiceCategoryConfiguration
        HasMaxLength(1024). Migrations: SqlServer Docs/Database/SQL/20260615_Add_ServiceCategory_ImageAIPrompt.sql,
        Postgres Docs/Postgres/22_add_servicecategory_imageaiprompt.sql (idempotent).
        DB APPLIED & VERIFIED on Supabase prod 2026-06-16 (varchar(1024), nullable) via Npgsql runner.
      API: NO new endpoint. The seed rides the existing catalog upsert (modal Save persists it), unlike
        Phase 4's dedicated endpoint (chosen because catalog rows may not exist yet in create mode).
        Extended (all positional, ImageAIPrompt added after ImageUrl):
          Request DTOs ServiceCategoryUpsertRequest / ServiceUpsertRequest (+ ImageAIPrompt string?).
          Commands Create/Update ServiceCategory + Create/Update Service (+ ImageAIPrompt param,
            validator MaximumLength(1024), handler trims & null-empties before assign).
          Response DTOs ServiceCategoryAdminResponse / ServiceAdminResponse (+ ImageAIPrompt string?),
            ServiceCatalogMapper now emits category.ImageAIPrompt / service.ImageAIPrompt.
          ServiceCatalogAdminController passes request.ImageAIPrompt into all four commands.
        GET /api/admin/services/catalog therefore returns imageAIPrompt on every category + service.
      Admin UI: ServiceCatalogManager.tsx — ImagePromptStudio added to BOTH the category modal and the
        service modal image rows. Category uses new image-prompts.ts composeCategoryPrompt /
        defaultCategorySubject (representative category hero shot, same 1280×720/16:9 + navy/gold +
        GLOBAL_QUALITY/NEGATIVE engine); service reuses composeServicePrompt / defaultServiceSubject.
        The studio's onSave stages the seed into the modal form state; it is persisted to the DB when
        the parent modal Save runs the upsert. Repository service-catalog-repository.ts: AdminCatalogCategory
        / AdminCatalogService gained imageAIPrompt (string|null); CategoryUpsertInput / ServiceUpsertInput
        carry imageAIPrompt.
      Validation status: Admin tsc --noEmit clean (2026-06-15); Coolzo.Application build 0 errors
        (Api copy-locked by running IIS Express only — no compiler errors). UI rendered-check of the two
        catalog Prompt modals PENDING (UNVERIFIED per §5b — auth wall).
